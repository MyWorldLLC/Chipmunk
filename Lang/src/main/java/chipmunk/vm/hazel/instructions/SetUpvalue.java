...
public final class SetUpvalue extends Instruction {

    protected final int local;

    public SetUpvalue(int sp, int local) {
        super(sp);
        this.local = local;
    }
    ...
    public int apply(Fiber fiber, int ip, int bp) {
        ...
        var upvalue = (Upvalue) heap.read(fiber.stack[bp + local]);
        upvalue.set(fiber.stack[bp + sp - 1]);
        return ip + 1;
    }
}
